	...
	While 1
		...
		Switch $nMsg
			...
			Case $cbAutorun
				...
				EndIf
			Case $btReg
				Local $FName = InputBox('Register IDM', 'Type your first name here: ', 'IDM trial reset', '', '', '130')
				Local $LName = InputBox('Register IDM', 'Type your last name here: ', 'IDM trial reset', '', '', '130')
				If @error <> 1 Then
					If StringLen($FName) = 0 Then $Name = 'First'
					If StringLen($LName) = 0 Then $Name = 'Last'
			...
			Case $btUpdate
				...
				MsgBox(262144, 'IDM trial reset', 'No Autoupdate Available!')
